...
function templeCard(temples) {
    const table = document.createElement("table")
    table.innerHTML = `<tr>
        <th colspan="2">${temples.templeName}</th></tr>
        <tr>
            <th>Location: </th>
            <td>${temples.location}<td>
        </tr>
        <tr>
            <th>Dedicated: </th>
            <td>${temples.dedicated}</td>
        </tr>
        <tr>
            <th>Size: </th>
            <td>${temples.area} sq ft</td>
        </tr>
        <tr>
            <td colspan="2"><img src="${temples.imageUrl}" alt="${temples.templeName}" loading="lazy"></td>
        `
    return table

};

function loadTempleCardples(filter) {
    const div = document.querySelector(".img-grid");
    div.innerHTML = "";

    temples
        .filter(filter)
        .forEach(temple => {
            const createTempleCard = templeCard(temple);
            div.appendChild(createTempleCard);
        });
}
